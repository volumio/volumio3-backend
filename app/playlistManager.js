...
class PlaylistManager {
  ...
  removeFromPlaylist(name, service, uri) {
    var self = this;
  ...
  removeFromFavourites(name, service, uri) {
    var self = this;
  ...
  removeFromRadioFavourites(name, service, uri) {
    var self = this;
  ...
  removeFromMyWebRadio(name, service, uri) {
    var self = this;
  ...
  commonAddToPlaylist(folder, name, service, uri, title) {
    var self = this;
    ...
    fs.exists(filePath, function (exists) {
      ...
      if (!exists) {
        ...
        fs.writeJson(folder + name, playlist, function (err) {
          if (err) { fileDefer.reject(new Error()); } else fileDefer.resolve();
        });
  ...
  saveJSONFile(localFolder, fileName, data) {
    var self = this;
    var defer = libQ.defer();
    ...
    if (this.commandRouter.sharedVars.get('myVolumio.cloudDeviceEnabled') === true && this.commandRouter.sharedVars.get('myvolumio.uid') !== '0') {
      ...
      fs.writeJson(localFolder + fileName, data, function (err) {
        if (err) { defer.reject(new Error()); } else defer.resolve();
      });
    }
  ...
  commonRemoveFromPlaylist(folder, name, service, uri) {
    var self = this;
  ...
  commonPlayPlaylist(folder, name) {
    var self = this;
  ...
  commonGetPlaylistContent(folder, name) {
    var defer = libQ.defer();

    var filePath = folder + name;

    fs.exists(filePath, function (exists) {
      if (!exists) { defer.resolve([]); } else {
        fs.readJson(filePath, function (err, data) {
          if (err) { defer.reject(new Error('Error reading playlist from ' + filePath)); } else {
            defer.resolve(data);
          }
        });
      }
  ...
  commonAddItemsToPlaylist(folder, name, data) {
    var self = this;
